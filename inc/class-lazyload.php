...
class CreateLazyImg {
	...
	public function lazyloadBackgroundImages( $html, $buffer ) {
		...
		if ( ! preg_match_all( '#<(?<tag>div|section|article|a|figure|li|span|header|footer)\s+(?<before>[^>]+[\'"\s])?style\s*=\s*([\'"])(?<styles>.*?)\3(?<after>[^>]*)>#is', $buffer, $elements, PREG_SET_ORDER ) ) {
			return $html;
		}
	...
	public function lazyloadVideos( $html, $buffer ) {
		if ( ! preg_match_all( '#<video(?<atts>\s.+)\s?>(?<sources>.*)</video>#iUs', $buffer, $videos, PREG_SET_ORDER ) ) {
			return $html;
		}
		...
		foreach ( $videos as $video ) {
			if ( $this->isExcluded( $video[0], $excluded ) ) {
				continue;
			}

			if ( preg_match_all( '#<source(?<atts>\s.+)>#iUs', $video['sources'], $sources, PREG_SET_ORDER ) ) {
				$sources = array_unique( $sources, SORT_REGULAR );
	...
	private function replaceImage( $image ) {
		...
		$image_lazyload = preg_replace( '/[\s|"|\'](srcset)\s*=\s*("|\')([^"|\']+)\2/i', ' data-lazy-$1=$2$3$2', $image_lazyload );

		$image_lazyload = preg_replace( '/[\s|"|\'](sizes)\s*=\s*("|\')([^"|\']+)\2/i', ' data-lazy-$1=$2$3$2', $image_lazyload );
	...
	private function replaceIframe( $iframe ) {
		...
		$iframe_lazyload  = str_replace( $iframe['atts'], $placeholder_atts . ' data-pre-lazyload="1" data-lazy-src="' . esc_url( $iframe['src'] ) . '"', $iframe[0] );
